...
void CacheLevel::set_policy(CacheReplacementPolicy p) {
  policy = p;
  // Reset stats or state if needed? Maybe not stats, but logical state
  // FIFO pointer is already valid
  // LRU/LFU counters will just build up from current state
}
